...
class User:
    def __init__(self, name):
        ...
        self.mistakes = 0
        self.accuracy = 100.0
    def set_score(self, missed, mistakes, accuracy):
        self.missed = missed
...
def write_title_screen(name_inputbox):
    ...
    name_text_surface = score_font.render("Name", True, FONT_COLOUR)

    game_window.blit(title_text_surface, ((WIDTH // 2) - (title_font_size[0] // 2), 50))
    game_window.blit(instruction_text_surface, ((WIDTH // 2) - (instruction_font_size[0] // 2), HEIGHT - 50 - instruction_font_size[1]))
    game_window.blit(name_text_surface, ((WIDTH // 4) - (name_font_size[0] // 2), (HEIGHT // 2)))
    name_inputbox.render(game_window)

# Title Screen
def title_screen():
    game_over = True
    name = ""
    name_inputbox = ib.InputBox((WIDTH//2), (HEIGHT//2), 140, 32)
    while game_over == True:
        ...
        for event in events:
            if event.type == QUIT:
                game_over = False
            name = name_inputbox.handle_event(event)
            if event.type == KEYDOWN:
                if event.key == K_ESCAPE:
                    game_over = False
                elif event.key == K_RETURN:
                    # DB 체크 후 중복된 이름이 있으면 체크할 필요가 있음 
                    # TODO: DB 구현부 추가
                    user = User(name)
                    game(user)
        write_title_screen(name_inputbox)
